 ...
 */
public class PagamentoPedidoView {
     ...
     * @param pedido A corrida que está sendo paga.
     * @return A FormaPagamento escolhida ou null se o pagamento for cancelado.
     */
    public static FormaPagamento executar(Pedido pedido) {
        while (true) {
            ...
            try {
                ...
                if (escolha > 0 && escolha <= FormaPagamento.values().length) {
                    ...
                    pedido.setFormaPagamento(formaEscolhida);

                    if (processarPagamento(pedido)) {
                        ps.atualizarFormaPagamento(pedido, formaEscolhida);
    ...
    private static boolean processarPagamento(Pedido pedido) {
        switch (pedido.getFormaPagamento()) {
            ...
            case PIX:
                return exibirInstrucoesPix(pedido);
            case DINHEIRO:
                return exibirInstrucoesDinheiro(pedido);
            default:
                return false;
        }
    }
    ...
    private static boolean exibirInstrucoesPix(Pedido pedido) {
    ...
    private static boolean exibirInstrucoesDinheiro(Pedido pedido) {
        ViewUtils.limparConsole();
